...
namespace Online2.DAL.Concrete.Context.Configurations
{
    public class FoodConfiguration : BaseConfiguration<Food>
    {
        public override void Configure(EntityTypeBuilder<Food> builder)
        {
            ...
            builder.Property(x => x.FoodName).IsRequired().HasMaxLength(80);

            builder.HasIndex(x => x.FoodName).IsUnique();

            builder.Property(x => x.Description).IsRequired().HasMaxLength(200);

            builder.Property(x => x.Calorie).HasPrecision(4).IsRequired();

            builder.HasOne(x => x.Category).WithMany(x => x.Foods).HasForeignKey(x => x.CategoryId).OnDelete(DeleteBehavior.Restrict);

            builder.HasMany(x => x.MealFoods).WithOne(x => x.Food).HasForeignKey(x => x.FoodId).OnDelete(DeleteBehavior.NoAction);

            builder.HasData(
                new Food { Id = 1, FoodName = "Süt", Description = "1 su bardağı,200 ml", Calorie = 11400, CategoryId = 1, CreateOn = DateTime.Now, isActive = true },
